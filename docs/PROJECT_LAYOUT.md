# Gryce Engine — Resource Path & Project Layout

## 1. `res:/` 虚拟资源根

`res:/` 是 Gryce Engine 的虚拟资源根路径。它指向"基于本引擎制作的游戏项目"的根目录，而非引擎仓库根目录。
...
引擎仓库本身包含示例项目，但 `res:/` 的语义目标是任意游戏项目根目录：
...
GryceGC（Gryce Game Compiler）是将来用于打包游戏资源的编译器/管线工具。本阶段不实现 GryceGC，但示例项目须预先符合 GryceGC-A 布局标准，以便未来平滑迁移。
...
│   ├── audio/                 # 音效/音乐
│   ├── fonts/                 # 字体
│   ├── shaders/               # 着色器源（未来 GryceGC 编译为 SPIR-V/DXBC/Metal IR）
│   └── tilesets/              # 2D 瓦片集
├── physics/                   # 物理材质、碰撞体预设（预留）
└── cache/                     # GryceGC 输出缓存（gitignore）
```

### 2.2 `project.gryce` 格式
