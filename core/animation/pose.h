...
GRYCE_API Pose sample_local_pose(const Skeleton& skeleton, const AnimationClip* clip,
...
GRYCE_API std::vector<math::Matrix4f> compute_global_pose(const Skeleton& skeleton, const Pose& local);

// global pose × inverse bind matrix → 蒙皮 palette（skinning 矩阵数组）。
// palette[i] = global[i] * inverse_bind_matrix[i]
GRYCE_API std::vector<math::Matrix4f> compute_skin_palette(const Skeleton& skeleton,
...
GRYCE_API std::vector<math::Matrix4f> evaluate_skin_palette(const Skeleton& skeleton,
