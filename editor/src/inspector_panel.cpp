...
void InspectorPanel::render() {
    ...
    if (!entity) {
        ImGui::TextUnformatted(tr("Select an entity to inspect"));
        ImGui::End();
        return;
    }
...
void InspectorPanel::render_entity_header(scene::Entity* entity) {
    ...
    ImGui::TextUnformatted(tr("Entity"));
    ImGui::SameLine();
    ...
    ImGui::BeginDisabled(play_mode_);
    FluentCheckbox enabled_check(&entity->enabled);
    if (enabled_check.Draw(tr("Enabled"), fluent_theme())) {
        entity->mark_dirty();
    }
    ImGui::EndDisabled();
}
...
void InspectorPanel::render_component(scene::Entity* entity, components::Component* component) {
    ...
    if (!type_info) {
        ImGui::TextDisabled(tr("No reflection metadata for %s"),
                            component->type());
        ImGui::PopID();
        return;
    }
...
void InspectorPanel::render_field(components::Component* component,
    ...
    case reflection::FieldType::Bool: {
        ...
        FluentCheckbox check(&value);
        changed = check.Draw(field.display_name.c_str(), fluent_theme());
    ...
    case reflection::FieldType::Int: {
        ...
        if (field.has_range) {
            ...
            FluentSlider s(&fv, static_cast<float>(field.range_min),
                           static_cast<float>(field.range_max), "%.0f");
            s.Draw(field.display_name.c_str(), fluent_theme());
            if (s.ValueChanged()) {
                value = static_cast<int>(fv);
    ...
    case reflection::FieldType::Float: {
        ...
        if (field.has_range) {
            FluentSlider s(&value, field.range_min, field.range_max);
            s.Draw(field.display_name.c_str(), fluent_theme());
            if (s.ValueChanged()) {
                reflection::write_field(component, field, value);
                changed = true;
...
void InspectorPanel::draw_add_component_popup(scene::Entity* entity) {
    ImGui::BeginDisabled(play_mode_);
    FluentButton add_btn(/*accent=*/true);
    if (add_btn.Draw(tr("Add Component"), fluent_theme())) {
    ...
    ImGui::InputTextWithHint("##component_filter", tr("Search..."), filter,
                             sizeof(filter));
