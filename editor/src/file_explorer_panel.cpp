...
void FileExplorerPanel::render_as_tab(bool file_system_tab) {
    if (file_system_tab) {
        ...
        if (ImGui::BeginTabBar("explorer_tabs")) {
            if (ImGui::BeginTabItem("\xe6\x96\x87\xe4\xbb\xb6\xe7\xb3\xbb\xe7\xbb\x9f")) { // FileSystem
                tab = 0;
                ImGui::EndTabItem();
            }
            if (ImGui::BeginTabItem("\xe5\x8e\x86\xe5\x8f\xb2\xe8\xae\xb0\xe5\xbd\x95")) { // History
                tab = 1;
                ImGui::EndTabItem();
            }
            ImGui::EndTabBar();
        ...
        if (tab != 0) {
            ImGui::TextDisabled("History tab is not yet implemented");
            ImGui::End();
            return;
        }
    } else {
    ...
    if (root_.empty()) {
        ImGui::TextUnformatted("Project root is not configured");
        ImGui::End();
        return;
    }
...
void FileExplorerPanel::render_list() {
    ...
    if (!fs::exists(current_, ec) || !fs::is_directory(current_, ec)) {
        ImGui::TextColored(ImVec4(0.8f, 0.35f, 0.30f, 1.0f),
                           "Directory not found: %s", current_.string().c_str());
        return;
    }
