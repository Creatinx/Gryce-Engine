...
// ===========================================================================
// scene_pbr_ssbo.frag — SSBO-based PBR fragment shader
// 与 scene_pbr.frag 功能相同，但使用 SSBO 替代 uniform 数组传递光源数据。
// 需要 OpenGL 4.3+ 或 Vulkan。
// SSBO 布局与 light_storage_impl.cpp 的 _pack_lights 完全一致：
//   每光源 20 个 float（80 字节），std430 对齐
// ===========================================================================
...
// ---------------------------------------------------------------------------
// SSBO 光源数据 — 精确匹配 CPU 端 _pack_lights 格式
// 每光源 20 个 float（80 字节），packed 为 5 个 vec4
//   type_pad:        x=type (0=Dir/1=Point/2=Spot), yzw=padding
//   position:        xyz=position, w=padding
//   direction:       xyz=direction, w=padding
//   color_intensity: xyz=color, w=intensity
//   params:          x=range, y=cos(outer), z=cos(inner), w=spot_softness
// ---------------------------------------------------------------------------
struct LightData {
    vec4 type_pad;
    vec4 position;
...
void main() {
    ...
    int num_lights = min(uLightCount, 256);
    for (int i = 0; i < num_lights; ++i) {
        LightData ld = lights[i];
        int type = int(ld.type_pad.x);
        vec3 pos = ld.position.xyz;
        vec3 dir = ld.direction.xyz;
        vec3 color = ld.color_intensity.xyz;
        float intensity = ld.color_intensity.w;
        vec4 params = ld.params;
        vec3 L;
        vec3 radiance;
        float shadow = 1.0;
        ...
        if (type == 0) {
            ...
            radiance = color * intensity * attenuation * cutoff;
        } else if (type == 2) {
            // ---- 聚光灯 ----
            vec3 to_light = pos - vFragPos;
            float dist = length(to_light);
            ...
            // 聚光锥形衰减
            float cos_angle = dot(-L, normalize(dir));
            float cos_outer = params.y;
            float cos_inner = params.z;
