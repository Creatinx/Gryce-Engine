...
uniform vec4 uCascadeFarBlend;    // x = far, y = 级联混合带宽比例
uniform vec4 uCascadeBias;        // xyz = 每级基础 bias
...
// 各向异性 GGX（Filament 风格），沿切线/副切线方向的粗糙度分离
...
// 级联边界（相机空间距离）：boundary(0)=near，boundary(count)=far
...
float pcss_cascade(int cascade, vec3 proj_coords, float bias, float normal_dot_light) {
    ...
    // 1. Blocker search：统计比 receiver 更靠近光源的深度平均值
...
float cascade_shadow_vsm_esm(vec3 frag_pos, vec3 normal, vec3 light_dir, out int out_cascade) {
    ...
    if (uShadowMode == 1) {
        // VSM: Chebyshev 不等式
        if (cascade == 0)      lit = vsm_sample(uVSMTexture0, sm_uv, frag_depth);
        else if (cascade == 1) lit = vsm_sample(uVSMTexture1, sm_uv, frag_depth);
        else if (cascade == 2) lit = vsm_sample(uVSMTexture2, sm_uv, frag_depth);
        else                   lit = vsm_sample(uVSMTexture3, sm_uv, frag_depth);
    } else {
        // ESM: 指数深度比较
        if (cascade == 0)      lit = esm_sample(uVSMTexture0, sm_uv, frag_depth, uESMExponent);
        else if (cascade == 1) lit = esm_sample(uVSMTexture1, sm_uv, frag_depth, uESMExponent);
        else if (cascade == 2) lit = esm_sample(uVSMTexture2, sm_uv, frag_depth, uESMExponent);
        else                   lit = esm_sample(uVSMTexture3, sm_uv, frag_depth, uESMExponent);
    }
    ...
    if (cascade < uCascadeCount - 1) {
        ...
        if (t > 0.0) {
            ...
            if (p2.z <= 1.0 && p2.x >= 0.0 && p2.x <= 1.0 && p2.y >= 0.0 && p2.y <= 1.0) {
                if (uShadowMode == 1) {
                    if (c2 == 0)      lit2 = vsm_sample(uVSMTexture0, p2.xy, p2.z);
                    else if (c2 == 1) lit2 = vsm_sample(uVSMTexture1, p2.xy, p2.z);
                    else if (c2 == 2) lit2 = vsm_sample(uVSMTexture2, p2.xy, p2.z);
                    else              lit2 = vsm_sample(uVSMTexture3, p2.xy, p2.z);
                } else {
                    if (c2 == 0)      lit2 = esm_sample(uVSMTexture0, p2.xy, p2.z, uESMExponent);
                    else if (c2 == 1) lit2 = esm_sample(uVSMTexture1, p2.xy, p2.z, uESMExponent);
                    else if (c2 == 2) lit2 = esm_sample(uVSMTexture2, p2.xy, p2.z, uESMExponent);
                    else              lit2 = esm_sample(uVSMTexture3, p2.xy, p2.z, uESMExponent);
                }
            }
            lit = mix(lit, lit2, t);
        }
...
void main() {
    ...
    for (int i = 0; i < uLightCount; ++i) {
        ...
        if (uLightType[i] == 0) {
            ...
            // 点光 / 聚光：物理正确的 inverse-square 衰减 + 平滑截止
            ...
            // 平滑截止：range*0.7 处开始衰减到 0，避免硬边界。
            // 注意 edge0 < edge1（否则是 GLSL 未定义行为）。
        ...
        if (abs(uAnisotropy) > 0.01) {
            // 各向异性方向：T/B 按 uAnisotropyRotation 旋转
            vec3 T = vTBN[0];
            vec3 B = vTBN[1];
        ...
        // Clearcoat：独立清漆层（车漆/塑料），固定 F0=0.04
        if (uClearcoat > 0.001) {
        ...
        // Sheen：织物光泽（D_Charlie 近似，能量按 1-F0 衰减）
        if (uSheen > 0.001) {
            float NdotH = max(dot(N, H), 0.0);
            float a = 0.3;
