...
// 后处理 push 常量：只取 blur 方向（PostProcessPushData 中 blur_direction 位于 offset 224）
layout(push_constant, std430) uniform PushData {
    layout(offset = 224) vec2 uBlurDirection;
} pc;
...
void main() {
    ...
    for (int i = -2; i <= 2; ++i) {
        vec2 offset = vec2(float(i)) * pc.uBlurDirection * texel;
