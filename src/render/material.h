...
public:
    ...
    // 便捷设置：基础颜色（无 albedo 贴图时使用）
    void set_albedo(const math::Vector3f& color) { albedo_color = color; }
    void set_roughness(float value) { roughness = value; }
    void set_metallic(float value) { metallic = value; }
