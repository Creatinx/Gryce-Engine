...
void RendererCanvasRenderRD::set_scissor(int x, int y, int w, int h) {
    if (ctx_) {
        ctx_->set_scissor(x, y, w, h);
    }
}

void RendererCanvasRenderRD::reset_scissor() {
    if (ctx_) {
        // 恢复全屏 scissor：使用当前视口大小
        const math::Vector2f size = screen_size();
